...
- **`code/masterplan.py` (build stage 4/11)** costs a **520 m²** programme — one 160 m² casa comunal plus **three 120 m²** satellites — on parcels H, G and N, with a phased schedule and an indicative site layout honouring the 30 m ronda hídrica, gravity-fed water and septic separation. Karst adds a 45 M contingency on G. *(Satellites were raised from 80 to 120 m² on 2026-08-08 at the group's request — 80 m² reads as a guest cabin, not a house you retire into.)* The script now also writes `outputs/masterplan_headline.json`, which `build_overview.py` reads for every build figure on the page, so a change to the programme can no longer leave stale numbers in the prose.
...
- **Result (520 m² programme):** on H, all-in ≈ **COP 3,320 M** (≈ **COP 1,107 M per household** across three, ≈ EUR 257k); phases 0–2 put the group on the land, living, at ≈ **COP 2,220 M** with contingency (≈ COP 740 M each). G ≈ 3,020 M, N ≈ 4,481 M. Running costs ≈ COP 26.4 M/yr including a part-time caretaker (≈ 8.8 M each).
